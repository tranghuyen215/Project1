	...
	if ($objValid->isValid()) {
		...
		if ($objCatalogue->addProduct($objValid->_post)) {
			...
			if ($objUpload->upload(CATALOGUE_PATH)) {
				$objCatalogue->updateCategory(array('image' => $objUpload->_names[0]), $objCatalogue->_id);
...
<form action="" method="post" enctype="multipart/form-data">
	...
	<table cellpadding="0" cellspacing="0" border="0" class="tbl_insert">
		...
		<tr>
			...
			<tr>
			<th><label for="pre_price">Pre_price: *</label></th>
			<td>
				<?php echo $objValid->validate('pre_price'); ?>
				<input type="text" name="pre_price" id="pre_price" 
					value="<?php echo $objForm->stickyText('pre_price'); ?>" class="fld_price" />
			</td>
		</tr>
