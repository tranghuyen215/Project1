<?php
class PayPal { //lớp này để làm việc với paypal

	// environment
	private $_environment = 'sandbox'; //biến environment: hộp thử nghiệm
	//khi nào ko thử nghiệm nữa mà dùng chính thức thì thay bằng xâu rỗng
	
	// urls
	private $_url_production = 'https://www.paypal.com/cgi-bin/webscr'; 
	private $_url_sandbox = 'https://www.sandbox.paypal.com/cgi-bin/webscr';//web thử nghiệm của paypal
	
	// url to be used
	private $_url; //lưu đúng trang đang dùng để khi quay lại
	
	// transaction type : 
	// _xclick = buy now buttons
	// _cart = basket
	private $_cmd; //kiểu giao dịch: 2 kiểu là xclick (nút bấm thanh toán) và cart(thanh toán theo giỏ hàng)
	
	// all products array
	private $_products = array(); //những sản phẩm cần thanh toán nằm trong đây
	
	// all input fields array
	private $_fields = array(); //trường cho khách hàng nhập vào
	
	// your paypal id
	private $_business = 'tranthieu2001@yahoo.com'; //= paypal_id mà ta đăng kí
	
	// page style
	private $_page_style = 'Test';//hình thức trang thanh toán, paypal cho phép ta đặt theme thanh toán
	//có thể tự tạo theme cho mình
	// return url
	private $_return; //trả lại biến url
	
	// cancel url
	private $_cancel_payment; 
	
	// notify url (IPN)
	private $_notify_url;//khi thanh toán thành công thì paypal gửi lại thông báo IPN
	
	// currency code
	private $_currency_code = 'GBP'; //mã tiền (VND,Dollar,v..v.)
	
	// tax / vat amount for _cart
	public $_tax_cart = 0; //tax và vat của giỏ hàng
	
	// tax / vat amount for _xclick
	public $_tax = 0; //tax và vat của xclick
	...
	public $_populate = array();//các thông tin của khách hàng
	
	// data received from paypal
	private $_ipn_data = array(); //các thông tin và paypal sẽ gửi lại nếu giao dịch thành công
	...
	private function addField($name = null, $value = null) { //thêm trường vào trong ... nhưng để ẩn đi
	...
	private function render() {//hiện ra đoạn html
